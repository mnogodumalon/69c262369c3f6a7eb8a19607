...
export default function ProjektverwaltungPage() {
  const navigate = useNavigate();
  const [records, setRecords] = useState<Projektverwaltung[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Projektverwaltung | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Projektverwaltung | null>(null);
  ...
  async function handleCreate(fields: Projektverwaltung['fields']) {
  ...
  async function handleUpdate(fields: Projektverwaltung['fields']) {
    ...
    await LivingAppsService.updateProjektverwaltungEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteProjektverwaltungEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Projektverwaltung im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Projektverwaltung suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('projektleiter_vorname')}>
                <span className="inline-flex items-center gap-1">
                  Vorname Projektleiter
                  {sortKey === 'projektleiter_vorname' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('projektleiter_nachname')}>
                <span className="inline-flex items-center gap-1">
                  Nachname Projektleiter
                  {sortKey === 'projektleiter_nachname' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('projektstatus')}>
                <span className="inline-flex items-center gap-1">
                  Projektstatus
                  {sortKey === 'projektstatus' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('budget')}>
                <span className="inline-flex items-center gap-1">
                  Budget (€)
                  {sortKey === 'budget' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/projektverwaltung/${record.record_id}`); }}>
                <TableCell className="font-medium">{record.fields.projektname ?? '—'}</TableCell>
                <TableCell>{record.fields.projektnummer ?? '—'}</TableCell>
                ...
                <TableCell>{record.fields.projektleiter_vorname ?? '—'}</TableCell>
                <TableCell>{record.fields.projektleiter_nachname ?? '—'}</TableCell>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{record.fields.projektstatus?.label ?? '—'}</span></TableCell>
                <TableCell>{record.fields.budget ?? '—'}</TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={12} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Projektverwaltung. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
