/**
 * ProjektverwaltungDialog — pre-generated create/edit dialog for Projektverwaltung.
 *
 ...
 * ProjektverwaltungDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Projektverwaltung['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<ProjektverwaltungDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Projektverwaltung, Kundenverwaltung, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Projektverwaltung';
...
/** Widened prefill type for ProjektverwaltungDialog.defaultValues — see file header. */
export type ProjektverwaltungDialogDefaults = Omit<Projektverwaltung['fields'], 'projektstatus'> & {
    projektstatus?: LookupValue | string;
  };

interface ProjektverwaltungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Projektverwaltung['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: ProjektverwaltungDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  projektstatus: LOOKUP_OPTIONS['projektverwaltung']?.['projektstatus'] ?? [],
};
...
export function ProjektverwaltungDialog({ open, onClose, onSubmit, defaultValues, recordId, kundenverwaltungList, enablePhotoScan = true, enablePhotoLocation = true }: ProjektverwaltungDialogProps) {
  const [fields, setFields] = useState<Partial<Projektverwaltung['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['projektname', 'kunde'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Projektverwaltung['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'projektverwaltung');
      await onSubmit(clean as Projektverwaltung['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "projektname": string | null, // Projektname\n  "projektnummer": string | null, // Projektnummer\n  "kunde": string | null, // Display name from Kundenverwaltung (see <available-records>)\n  "projektbeschreibung": string | null, // Projektbeschreibung\n  "startdatum": string | null, // YYYY-MM-DD\n  "enddatum": string | null, // YYYY-MM-DD\n  "projektleiter_vorname": string | null, // Vorname Projektleiter\n  "projektleiter_nachname": string | null, // Nachname Projektleiter\n  "projektstatus": LookupValue | null, // Projektstatus (select one key: "angebot" | "aktiv" | "abgeschlossen" | "pausiert" | "storniert") mapping: angebot=Angebot, aktiv=Aktiv, abgeschlossen=Abgeschlossen, pausiert=Pausiert, storniert=Storniert\n  "budget": number | null, // Budget (€)\n  "notizen": string | null, // Notizen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Projektverwaltung['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Projektverwaltung bearbeiten' : 'Projektverwaltung hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'projektbeschreibung': (
      <div key="projektbeschreibung" className="space-y-1.5">
        ...
        <Textarea
          ...
          placeholder="Umfang, Ziele, Besonderheiten, Anforderungen..."
    ...
    'projektleiter_vorname': (
      <div key="projektleiter_vorname" className="space-y-1.5">
        <Label htmlFor="projektleiter_vorname">Vorname Projektleiter</Label>
        <Input
          id="projektleiter_vorname"
          placeholder="z. B. Michael"
          value={fields.projektleiter_vorname ?? ''}
          onChange={e => setFields(f => ({ ...f, projektleiter_vorname: e.target.value }))}
        />
      </div>
    ),
    'projektleiter_nachname': (
      <div key="projektleiter_nachname" className="space-y-1.5">
        <Label htmlFor="projektleiter_nachname">Nachname Projektleiter</Label>
        <Input
          id="projektleiter_nachname"
          placeholder="z. B. Müller"
          value={fields.projektleiter_nachname ?? ''}
          onChange={e => setFields(f => ({ ...f, projektleiter_nachname: e.target.value }))}
        />
      </div>
    ),
    'projektstatus': (
      <div key="projektstatus" className="space-y-1.5">
        <Label htmlFor="projektstatus">Projektstatus</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.projektstatus) === 'angebot'}
            onClick={() => setFields(f => ({ ...f, projektstatus: (lookupKey(f.projektstatus) === 'angebot' ? undefined : 'angebot') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.projektstatus) === 'angebot'
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.projektstatus) === 'aktiv'}
            onClick={() => setFields(f => ({ ...f, projektstatus: (lookupKey(f.projektstatus) === 'aktiv' ? undefined : 'aktiv') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.projektstatus) === 'aktiv'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.projektstatus) === 'pausiert'}
            onClick={() => setFields(f => ({ ...f, projektstatus: (lookupKey(f.projektstatus) === 'pausiert' ? undefined : 'pausiert') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.projektstatus) === 'pausiert'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.projektstatus) === 'storniert'}
            onClick={() => setFields(f => ({ ...f, projektstatus: (lookupKey(f.projektstatus) === 'storniert' ? undefined : 'storniert') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.projektstatus) === 'storniert'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'notizen': (
      <div key="notizen" className="space-y-1.5">
        ...
        <Textarea
          id="notizen"
          placeholder="Interne Anmerkungen, Risiken, Abhängigkeiten..."
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.PROJEKTVERWALTUNG} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
