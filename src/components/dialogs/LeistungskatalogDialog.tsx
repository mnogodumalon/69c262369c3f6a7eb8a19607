...
export type LeistungskatalogDialogDefaults = Omit<Leistungskatalog['fields'], 'einheit' | 'waehrung'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  ...
  waehrung: LOOKUP_OPTIONS['leistungskatalog']?.['waehrung'] ?? [],
};
...
export function LeistungskatalogDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: LeistungskatalogDialogProps) {
  ...
  const REQUIRED_FIELDS = ['leistungsbezeichnung', 'einheit', 'standardpreis'] as const;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'leistungsbezeichnung': (
      <div key="leistungsbezeichnung" className="space-y-1.5">
        <Label htmlFor="leistungsbezeichnung">Leistungsbezeichnung <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
    ...
    'leistungsbeschreibung': (
      <div key="leistungsbeschreibung" className="space-y-1.5">
        <Label htmlFor="leistungsbeschreibung">Beschreibung</Label>
        <Textarea
          id="leistungsbeschreibung"
          placeholder="Was ist enthalten? Umfang, Qualität..."
    ...
    'standardpreis': (
      <div key="standardpreis" className="space-y-1.5">
        <Label htmlFor="standardpreis">Standardpreis (€) <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="standardpreis"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'standardpreis')}
          placeholder="z. B. 150,00"
          value={fields.standardpreis !== undefined ? fields.standardpreis : (computedValues['standardpreis'] ?? '')}
          onChange={e => setFields(f => ({ ...f, standardpreis: clampNumberValue(formEnhancements, 'standardpreis', e.target.value) }))}
        />
        {showErrors && !fields.standardpreis && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    ...
    'aktiv': (
      <div key="aktiv" className="space-y-1.5">
        <Label htmlFor="aktiv">Leistung aktiv</Label>
        <div className="flex items-center gap-2 pt-1">
          ...
          />
          <Label htmlFor="aktiv" className="font-normal">Leistung aktiv</Label>
        </div>
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"leistungsbezeichnung": "Leistungsbezeichnung", "leistungsbeschreibung": "Beschreibung", "einheit": "Einheit", "standardpreis": "Standardpreis (€)", "waehrung": "Währung", "aktiv": "Leistung aktiv"};
  const CURRENCY_KEYS = new Set<string>(["standardpreis"]);
