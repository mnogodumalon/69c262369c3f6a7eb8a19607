...
import type { Rechnungsverwaltung, Rechnungspositionen, Projektverwaltung, Leistungskatalog, Kundenverwaltung, CreateRechnungsverwaltung, CreateRechnungspositionen, CreateProjektverwaltung, CreateLeistungskatalog, CreateKundenverwaltung } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getRechnungsverwaltungEntry(id: string): Promise<Rechnungsverwaltung | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Rechnungsverwaltung;
    return enrichLookupFields([record], 'rechnungsverwaltung')[0];
  }
  static async createRechnungsverwaltungEntry(fields: CreateRechnungsverwaltung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.RECHNUNGSVERWALTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'rechnungsverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async updateRechnungsverwaltungEntry(id: string, fields: Partial<CreateRechnungsverwaltung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.RECHNUNGSVERWALTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'rechnungsverwaltung') });
    return { ...data, record_id: data.id };
  }
  ...
  static async getRechnungspositionenEntry(id: string): Promise<Rechnungspositionen | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Rechnungspositionen;
    return enrichLookupFields([record], 'rechnungspositionen')[0];
  }
  static async createRechnungspositionenEntry(fields: CreateRechnungspositionen): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.RECHNUNGSPOSITIONEN}/records`, { fields: cleanFieldsForApi(fields as any, 'rechnungspositionen') });
    return { ...data, record_id: data.id };
  }
  static async updateRechnungspositionenEntry(id: string, fields: Partial<CreateRechnungspositionen>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.RECHNUNGSPOSITIONEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'rechnungspositionen') });
    return { ...data, record_id: data.id };
  }
  ...
  // --- PROJEKTVERWALTUNG ---
  static async getProjektverwaltung(): Promise<Projektverwaltung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.PROJEKTVERWALTUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Projektverwaltung[];
    return enrichLookupFields(records, 'projektverwaltung');
  }
  static async getProjektverwaltungEntry(id: string): Promise<Projektverwaltung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.PROJEKTVERWALTUNG}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Projektverwaltung;
    return enrichLookupFields([record], 'projektverwaltung')[0];
  }
  static async createProjektverwaltungEntry(fields: CreateProjektverwaltung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.PROJEKTVERWALTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'projektverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async updateProjektverwaltungEntry(id: string, fields: Partial<CreateProjektverwaltung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.PROJEKTVERWALTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'projektverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async deleteProjektverwaltungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.PROJEKTVERWALTUNG}/records/${id}`);
  }
